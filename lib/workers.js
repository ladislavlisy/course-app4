...
workers.checkTimerSeconds = 5;

workers.rotateLogsTimerSeconds = 60 * 60 * 24;

// Lookup all checks, get their data, send to the validator
        ...
        checks.forEach(function(check){
          _data.read('checks', check, function(err,originalCheckData){
            if (!err && originalCheckData){
              // Puass the check data to the check validator, and let that function continue or log the error(s) as needed
...
workers.validateCheckData = function(originalCheckData){
  ...
  originalCheckData.lastChecked = typeof(originalCheckData.lastChecked) == 'number' && originalCheckData.lastChecked > 0 ? originalCheckData.lastChecked : 0;
...
workers.loop = function(){
  ...
  },1000 * workers.checkTimerSeconds);
};
...
workers.logRotationLoop = function(){
  setInterval(function(){
    workers.rotateLogs();
  },1000 * workers.rotateLogsTimerSeconds);
}
